...
async function getNavigationAccess(user){
 if(!user)return{signedIn:false,isOwner:false,isStaff:false,isManager:false,isLending:false,permissions:[]};
 const[bootstrapSnap,staffSnap,permissionsSnap]=await Promise.all([getDoc(doc(db,'system','bootstrap')),getDoc(doc(db,'staffProfiles',user.uid)),getDoc(doc(db,'staffPermissions',user.uid))]);
 const isOwner=bootstrapSnap.exists()&&bootstrapSnap.data().ownerUid===user.uid,staff=staffSnap.exists()?staffSnap.data():null,permissions=permissionsSnap.exists()?permissionsSnap.data().permissions||[]:[],isStaff=isOwner||Boolean(staff&&staff.staffStatus!=='terminated'),isManager=isOwner||permissions.includes('staff.manage')||permissions.includes('owner.override_all')||managementTitle(staff?.rankName),isLending=isOwner||permissions.some(p=>['applications.view','applications.review','applications.claim','applications.approve','applications.deny','loans.view','owner.override_all'].includes(p));
 return{signedIn:true,isOwner,isStaff,isManager,isLending,permissions};
}
const routeRules={'/owner-control':a=>a.isOwner,'/account-management':a=>a.isOwner,'/staff-management':a=>a.isManager,'/departments':a=>a.isStaff,'/staff':a=>a.isStaff,'/career-center':a=>a.isStaff,'/lending':a=>a.isLending,'/economy-center':a=>a.signedIn,'/academy-center':a=>a.signedIn,'/careers':a=>a.signedIn,'/loans':a=>a.signedIn,'/applications':a=>a.signedIn,'/progression':a=>a.signedIn};
