...
export const SCORE_TIERS = [
  { min: 780, name: 'Elite' },
  { min: 720, name: 'Excellent' },
  { min: 650, name: 'Strong' },
  { min: 580, name: 'Fair' },
  { min: 500, name: 'Developing' },
  { min: 300, name: 'Critical' },
];
...
export function buildProgression(portal, banking) {
  ...
  return {
    ...
    totalBalance,
    milestones,
    completedMilestones: milestones.filter((item) => item.complete).length,
...
export async function findCustomerById(db, customerId) {
  ...
  const [profileSnap, creditSnap] = await Promise.all([
    getDoc(doc(db, 'customerProfiles', userDoc.id)),
    getDoc(doc(db, 'creditProfiles', userDoc.id)),
  ]);
...
export async function applyOwnerProgressionOverride(db, ownerUid, target, values, reason) {
  ...
  if (!Number.isFinite(score) || score < 300 || score > 850) {
    throw new Error('Trust Score must be between 300 and 850.');
  }
  ...
  const overrideId = `OVR-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const [profileSnap, creditSnap, userSnap] = await Promise.all([
      transaction.get(profileRef),
      transaction.get(creditRef),
      transaction.get(userRef),
    ]);

    if (!userSnap.exists()) throw new Error('Customer identity record is missing.');

    const currentProfile = profileSnap.exists() ? profileSnap.data() : {};
    const currentCredit = creditSnap.exists() ? creditSnap.data() : {};
    const customerId = userSnap.data().customerId || target.user?.customerId;

    const previous = {
      trustScore: currentCredit.trustScore ?? null,
      reputation: currentProfile.reputation ?? null,
      accountStatus: currentProfile.accountStatus ?? null,
      classificationOverride: currentProfile.classificationOverride ?? null,
      standingOverride: currentProfile.standingOverride ?? null,
    };
    ...
    transaction.set(
      creditRef,
      {
        ...
        trustScore: score,
        tier: scoreTier(score),
        paymentHistory: currentCredit.paymentHistory ?? 100,
        debtUtilization: currentCredit.debtUtilization ?? 0,
        accountAgePoints: currentCredit.accountAgePoints ?? 0,
        applicationImpact: currentCredit.applicationImpact ?? 0,
        status: currentCredit.status || 'active',
        ownerOverride: true,
    ...
    transaction.set(
      profileRef,
      {
        uid: target.uid,
        customerId,
        username: currentProfile.username || userSnap.data().username || '',
        displayName: currentProfile.displayName || userSnap.data().displayName || userSnap.data().username || customerId,
        discordUsername: currentProfile.discordUsername || userSnap.data().discordUsername || '',
        reputation,
        accountStatus: values.restricted ? 'restricted' : 'active',
        classificationOverride: values.classification || null,
        standingOverride: values.standing || null,
        updatedAt: serverTimestamp(),
        updatedBy: ownerUid,
      },
      { merge: true },
    ...
    transaction.set(doc(db, 'progressionOverrides', overrideId), {
      overrideId,
      ownerUid,
      ...
      replacement: {
        trustScore: score,
        reputation,
        restricted: Boolean(values.restricted),
    ...
    transaction.set(doc(db, 'auditLogs', overrideId), {
      auditId: overrideId,
      actorUid: ownerUid,
      actorType: 'owner',
      action: 'progression.owner_override',
      targetType: 'customer',
      targetId: customerId,
      reason: reason.trim(),
      immutable: true,
